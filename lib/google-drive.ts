...
export async function uploadCashReceiptToGoogleDrive(
  buffer: Buffer,
  fileName: string,
  mimeType: string
) {
  const rootFolderId = driveFolderId(process.env.GOOGLE_DRIVE_CASH_RECEIPTS_FOLDER_ID, "0ABkOsn4uZjSQUk9PVA");
  const drive = getDriveClient();
  const expensesFolderId = await getOrCreateSubfolder(drive, rootFolderId, "Spese");
  ...
  try {
    ...
    console.error("Failed to set public read permissions on Google Drive cash receipt:", err);
  }
  ...
  return {
    ...
    imageUrl: directDriveImageUrl(fileId),
...
export async function uploadTaskImageToGoogleDrive(
  ...
  const rootFolderId = driveFolderId(
    process.env.GOOGLE_DRIVE_TASK_ROOT_FOLDER_ID || process.env.GOOGLE_DRIVE_DOCUMENTS_FOLDER_ID,
    "0ABkOsn4uZjSQUk9PVA"
  );
  const drive = getDriveClient();
  const taskFolderId = await getOrCreateSubfolder(drive, rootFolderId, "Task");
  ...
  try {
    ...
    console.error("Failed to set public read permissions on Google Drive task image:", err);
  }
