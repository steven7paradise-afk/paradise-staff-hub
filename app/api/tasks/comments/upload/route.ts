...
export async function POST(request: NextRequest) {
  ...
  const canUpload = managerRoles.has(session.user.role) || isAssignee || isCreator;
  if (!canUpload || (session.user.role === "RESPONSABILE" && session.user.sedeId !== task.location_id && !isAssignee && !isCreator)) {
