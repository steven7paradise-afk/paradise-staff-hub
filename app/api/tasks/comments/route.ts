...
export async function POST(request: NextRequest) {
  ...
  const canComment = managerRoles.has(session.user.role) || isAssignee || isCreator;
  if (!canComment || (session.user.role === "RESPONSABILE" && session.user.sedeId !== task.location_id && !isAssignee && !isCreator)) {
