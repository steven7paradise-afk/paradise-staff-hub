...
import { createSibillDraft, SIBILL_ANSWER_KEYS, SibillDraftError } from "@/lib/sibill-invoice";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const user = session?.user?.id
    ...
    : null;
  const canCreateDraft = user?.active
    ? await canEditForUser(prisma, "/invoices", user)
    : false;
  if (!user?.id || !canCreateDraft) {
