  ...
  async function printLabels(items: BarcodeLabel[]) {
    ...
    try {
      ...
      printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Etichette barcode</title><style>
        @page{size:50.8mm 25.4mm;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#111;font-family:Arial,sans-serif}.label{width:50.8mm;height:25.4mm;padding:1.5mm 2mm;overflow:hidden;page-break-after:always;break-after:page}.label:last-child{page-break-after:auto;break-after:auto}.label-front{display:flex;flex-direction:column}.barcode{min-height:0;flex:1}.barcode svg{display:block;width:100%;height:100%}.caption{overflow:hidden;text-align:center;font-size:9px;font-weight:800;line-height:3.5mm;white-space:nowrap;text-overflow:ellipsis}.label-back{--label-text:8px;display:flex;flex-direction:column;padding:1.5mm 2.2mm}.back-header{display:flex;height:4.2mm;align-items:center;justify-content:space-between;border-bottom:.25mm solid #111}.back-brand{font-size:var(--label-text);font-weight:900;letter-spacing:.45px}.type-pill{max-width:18mm;overflow:hidden;border:.2mm solid #111;border-radius:3mm;padding:.4mm 1.1mm;font-size:var(--label-text);font-weight:600;line-height:1;white-space:nowrap;text-overflow:ellipsis;text-transform:uppercase}.back-feature{display:flex;min-height:7.2mm;flex-direction:column;justify-content:center;border-bottom:.2mm solid #bbb}.back-feature span,.back-feature strong,.back-specs dt,.back-specs dd{font-size:var(--label-text);line-height:1.15}.back-feature span{font-weight:600;letter-spacing:.35px;text-transform:uppercase}.back-feature strong{overflow:hidden;font-weight:900;white-space:nowrap;text-overflow:ellipsis}.back-specs{display:grid;flex:1;grid-template-columns:repeat(var(--columns),minmax(0,1fr));align-items:center;margin:0}.back-specs div{min-width:0;padding:0 .75mm;border-left:.2mm solid #ddd}.back-specs div:first-child{border-left:0;padding-left:0}.back-specs div:last-child{padding-right:0}.back-specs dt,.back-specs dd{overflow:hidden;margin:0;white-space:nowrap;text-overflow:ellipsis}.back-specs dt{font-weight:600;text-transform:uppercase}.back-specs dd{margin-top:.5mm;font-weight:900}@media screen{body{display:flex;flex-direction:column;align-items:center;gap:6mm;padding:10mm}.label{border:1px dashed #bbb;box-shadow:0 3mm 8mm rgba(0,0,0,.08)}}
                ...
                <div>
                  ...
                  <div className="mx-auto flex aspect-[2/1] w-full max-w-[300px] flex-col rounded-xl bg-white px-3 py-2 text-black shadow-sm ring-1 ring-black/5">
                    <div className="flex h-6 shrink-0 items-center justify-between border-b border-black">
                      <div className="text-[9px] font-black tracking-[0.08em]">PARADISE BEAUTY</div>
                      <span className="max-w-[92px] truncate rounded-full border border-black px-2 py-0.5 text-[9px] font-medium uppercase">
                    ...
                    <div className="flex min-h-0 flex-1 flex-col justify-center border-b border-black/25 py-1">
                      <span className="text-[9px] font-medium uppercase tracking-[0.06em]">{previewFeatureField?.label || "Prodotto"}</span>
                      <strong className="truncate text-[9px] font-black leading-tight">
                    ...
                    <dl className="grid h-8 shrink-0 items-center" style={{ gridTemplateColumns: `repeat(${Math.min(Math.max(previewSpecFields.length, 1), 5)}, minmax(0, 1fr))` }}>
                      {previewSpecFields.map((field, index) => (
                        <div key={field.key} className={`min-w-0 px-1.5 ${index ? "border-l border-black/15" : "pl-0"}`}>
                          <dt className="truncate text-[9px] font-medium uppercase">{field.label}</dt>
                          <dd className="mt-0.5 truncate text-[9px] font-black leading-none">{detailValues[field.key]?.trim() || "—"}</dd>
                        </div>
                      ))}
                    </dl>
                  </div>
