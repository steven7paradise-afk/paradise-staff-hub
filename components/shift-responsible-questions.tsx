...
function TaskAnswer({ assignees, selected, status, onAnswer }: { assignees: TaskAssignee[]; selected?: string; status?: SaveStatus; onAnswer: (value: string) => void }) {
  ...
  const [taskUpdateStatus, setTaskUpdateStatus] = useState<SaveStatus | "idle">("idle");
  ...
  async function appendTaskUpdate() {
    const message = taskUpdate.trim();
    if (!saved.taskId || !message || taskUpdateStatus === "saving") return;
    setTaskUpdateStatus("saving");
    try {
      const response = await fetch("/api/tasks/comments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ taskId: saved.taskId, message }),
      });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Errore");
      setTaskUpdate("");
      setTaskUpdateStatus("saved");
    } catch {
      setTaskUpdateStatus("error");
    }
  }

  if (saved.taskId) {
    return <div className="mt-4 rounded-xl border border-[#78c85c]/35 bg-[#f1faed] p-4"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#dff4d6] text-[#347a27]"><Check className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-black text-[#2f5f27]">Task creata</p><p className="mt-1 text-xs font-semibold text-[#26331f]">{saved.taskTitle}</p><p className="mt-1 text-[9px] text-[#4f6849]">Assegnata a {saved.assignees.map((person) => person.name).join(", ")}</p>{saved.aiComment ? <div className="mt-3 rounded-lg bg-white/70 p-3"><p className="text-[8px] font-black uppercase tracking-wide text-[#16883a]">Commento IA nella task</p><p className="mt-1 line-clamp-3 whitespace-pre-wrap text-[9px] leading-relaxed text-[#4f5f4a]">{saved.aiComment}</p></div> : null}<div className="mt-4 border-t border-[#78c85c]/25 pt-4"><label className="block"><span className="text-[9px] font-black uppercase tracking-wide text-[#16883a]">Aggiungi alla stessa task</span><textarea value={taskUpdate} onChange={(event) => { setTaskUpdate(event.target.value); if (taskUpdateStatus !== "idle") setTaskUpdateStatus("idle"); }} rows={3} maxLength={2000} placeholder="Scrivi soltanto il nuovo aggiornamento…" className="mt-2 w-full resize-none rounded-xl border border-[#78c85c]/30 bg-white px-3 py-3 text-xs outline-none focus:border-[#2ed65d]" /></label><p className="mt-2 text-[9px] text-[#4f6849]">Il testo precedente rimane invariato. Questa nota sarà aggiunta in fondo e firmata con il tuo nome.</p><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><a href={`/tasks?task=${encodeURIComponent(saved.taskId)}`} className="inline-flex text-[9px] font-black text-[#16883a] underline">Apri la task</a><div className="flex items-center gap-2">{taskUpdateStatus !== "idle" ? <SaveStatusLine status={taskUpdateStatus} /> : null}<button type="button" onClick={appendTaskUpdate} disabled={!taskUpdate.trim() || taskUpdateStatus === "saving"} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#2ed65d] px-4 text-[9px] font-black text-white disabled:opacity-40"><Plus className="size-4" />{taskUpdateStatus === "saving" ? "Aggiunta…" : taskUpdateStatus === "error" ? "Riprova" : "Aggiungi alla task"}</button></div></div></div></div></div></div>;
  }
