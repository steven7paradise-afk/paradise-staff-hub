...
type Props = { currentUserId: string; currentUserName: string; recipients: InternalEmailRecipient[]; restrictedToLocation: string | null; focusMessageId?: string | null };
...
export function InternalEmailComposer({ currentUserId, currentUserName, recipients, restrictedToLocation, focusMessageId }: Props) {
  ...
  const loadFolder = useCallback(async (nextFolder: Folder, preferredId?: string | null) => {
    ...
    try {
      ...
      setSelectedId(targetId && data.messages.some((message: InternalMessage) => message.id === targetId) ? targetId : data.messages[0]?.id || null);
  ...
  function newMessage(options?: { recipientIds?: string[]; subject?: string; body?: string; draftId?: string | null; attachments?: Attachment[] }) {
    setSelectedRecipientIds(new Set(options?.recipientIds || [])); setSubject(options?.subject || ""); setBody(options?.body || ""); setDraftId(options?.draftId || null); setAttachments(options?.attachments || []); setCompose(true); setRecipientPicker(false); setError(""); setNotice("");
  }
  ...
  return (
    <div className="w-full overflow-hidden bg-white">
      ...
      <div className="grid w-full grid-cols-1 overflow-hidden lg:min-h-[calc(100dvh-68px)] lg:grid-cols-[220px_300px_minmax(0,1fr)] xl:h-dvh xl:min-h-0 xl:grid-cols-[76px_220px_300px_minmax(0,1fr)] 2xl:grid-cols-[84px_240px_340px_minmax(0,1fr)]">
        ...
        <aside className="border-b border-[#EEDFE6] bg-[linear-gradient(180deg,#FFF4FA_0%,#FDECF4_48%,#FFF9FC_100%)] p-3 lg:border-b-0 lg:border-r lg:p-4">
          <div className="px-2 py-3"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#B63F73]">Paradise</p><h2 className="mt-1 text-2xl font-black text-[#211A1E]">Email</h2></div>
          <button type="button" onClick={() => newMessage()} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#211E20] text-sm font-black text-white shadow-[0_12px_30px_rgba(33,30,32,.18)] active:scale-[.98]"><PenLine className="size-4" /> Nuova email</button>
          <nav className="mt-4 flex gap-1.5 overflow-x-auto pb-1 lg:mt-5 lg:block lg:space-y-1.5 lg:overflow-visible lg:pb-0">
            {folderItems.map((item) => { const Icon = item.icon; const count = countFor(item.id); return (
              <button key={item.id} type="button" onClick={() => { setFolder(item.id); setCompose(false); }} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-2xl px-3 text-left text-xs font-bold transition lg:min-h-12 lg:w-full lg:gap-3 lg:text-sm ${folder === item.id && !compose ? "bg-white text-[#9F3263] shadow-[0_8px_22px_rgba(126,55,87,.10)]" : "text-[#56434C] hover:bg-white/65"}`}>
          ...
          {restrictedToLocation ? <div className="mt-4 rounded-2xl border border-[#EACFDC] bg-white/70 p-3 text-[10px] font-bold leading-4 text-[#85596D] lg:mt-8">Invii limitati a<br /><strong>{restrictedToLocation}</strong></div> : null}
        </aside>

        <section className="flex min-h-[360px] flex-col border-b border-[#E9E2E5] bg-[#FFFDFE] lg:min-h-0 lg:border-b-0 lg:border-r">
          <div className="border-b border-[#EFE6EA] p-4">
            <div className="flex items-center justify-between"><h3 className="text-xl font-black capitalize text-[#211A1E]">{folderItems.find((item) => item.id === folder)?.label}</h3><button type="button" onClick={() => newMessage()} className="grid size-9 place-items-center rounded-full bg-[#211E20] text-white"><PenLine className="size-4" /></button></div>
            <label className="relative mt-4 block"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-black/30" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cerca" className="h-11 w-full rounded-2xl border border-[#E9DDE3] bg-[#FAF7F9] pl-10 pr-4 text-sm font-semibold outline-none focus:border-[#D96F9E]" /></label>
            {folder !== "sent" && folder !== "drafts" ? <div className="mt-3 grid grid-cols-3 rounded-2xl bg-[#F4EEF1] p-1">{(["all", "read", "unread"] as const).map((value) => <button key={value} type="button" onClick={() => setReadFilter(value)} className={`min-h-8 rounded-xl text-[9px] font-black ${readFilter === value ? "bg-[#211E20] text-white shadow-sm" : "text-black/45"}`}>{value === "all" ? "Tutte" : value === "read" ? "Lette" : "Non lette"}</button>)}</div> : null}
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto">
            {loading ? <div className="grid h-48 place-items-center"><Loader2 className="size-6 animate-spin text-[#B43A70]" /></div> : visibleMessages.length ? visibleMessages.map((message) => {
              ...
              return <button key={message.id} type="button" onClick={() => folder === "drafts" ? newMessage({ recipientIds: message.draftRecipientIds, subject: message.subject, body: message.body, draftId: message.id, attachments: message.attachments }) : void selectMessage(message)} className={`w-full border-b border-[#F0E8EC] p-4 text-left transition ${active ? "bg-[#FFF0F7]" : "bg-white hover:bg-[#FFFAFC]"}`}>
                <div className="flex items-center gap-2">{contactPhoto ? <img src={contactPhoto} alt={contact?.name || "Profilo"} className="size-9 shrink-0 rounded-full object-cover" /> : <span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#F3DFE8] text-[10px] font-black text-[#8E315D]">{initials(contact?.name || "P")}</span>}<span className={`size-2 rounded-full ${message.read ? "bg-black/12" : "bg-emerald-500"}`} /><span className={`min-w-0 flex-1 truncate text-sm ${message.read ? "font-bold text-[#493A42]" : "font-black text-[#211A1E]"}`}>{senderName}</span><span className="text-[10px] font-bold text-black/35">{formatMailDate(message.updatedAt || message.createdAt)}</span></div>
                <p className="mt-2 truncate text-xs font-black text-[#352930]">{message.subject}</p><p className="mt-1 line-clamp-2 text-[11px] font-medium leading-4 text-black/42">{message.body || "Nessun testo"}</p>
                <div className="mt-2 flex gap-2">{message.starred ? <Star className="size-3.5 fill-amber-400 text-amber-400" /> : null}{message.attachments.length ? <Paperclip className="size-3.5 text-[#B43A70]" /> : null}</div>
              </button>;
        ...
        <main className="relative min-h-[560px] bg-white lg:min-h-0">
          {compose ? (
            <div className="flex h-full flex-col">
              <div className="flex items-center justify-between border-b border-[#EEE4E8] px-6 py-5"><div><p className="text-[9px] font-black uppercase tracking-[.2em] text-[#B43A70]">Composizione</p><h3 className="mt-1 text-2xl font-black text-[#211A1E]">Nuova email</h3></div><button type="button" onClick={() => { setCompose(false); resetComposer(); }} className="grid size-10 place-items-center rounded-full bg-black/5"><X className="size-5" /></button></div>
              <div className="flex-1 overflow-y-auto p-6">
                <div className="relative">
                  <button type="button" onClick={() => setRecipientPicker((value) => !value)} className="flex min-h-12 w-full flex-wrap items-center gap-2 rounded-2xl border border-[#E8DCE2] bg-[#FFFBFD] px-3 text-left">
                    <span className="text-xs font-black text-black/35">A:</span>{selectedRecipients.length ? selectedRecipients.map((recipient) => <span key={recipient.id} className="inline-flex items-center gap-1.5 rounded-full bg-[#F8DCE9] py-1 pl-1 pr-2.5 text-[10px] font-black text-[#94305E]">{personPhoto(recipient) ? <img src={personPhoto(recipient)} alt="" className="size-5 rounded-full object-cover" /> : <span className="grid size-5 place-items-center rounded-full bg-white/70 text-[8px]">{initials(recipient.name)}</span>}{recipient.name}</span>) : <span className="text-sm font-semibold text-black/30">Seleziona destinatari</span>}
                  </button>
                ...
                <input value={subject} onChange={(event) => setSubject(event.target.value)} maxLength={160} placeholder="Oggetto" className="mt-3 h-14 w-full rounded-2xl border border-[#E8DCE2] bg-[#FFFBFD] px-4 text-sm font-black outline-none focus:border-[#D96F9E]" />
                <textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={10000} placeholder="Scrivi il messaggio..." className="mt-3 min-h-72 w-full resize-y rounded-2xl border border-[#E8DCE2] bg-white p-4 text-sm font-medium leading-6 outline-none focus:border-[#D96F9E]" />
              ...
              <div className="flex flex-wrap items-center gap-2 border-t border-[#EEE4E8] p-5">
            ...
            <div className="flex h-full flex-col">
              <div className="sticky top-0 z-20 flex flex-nowrap items-center gap-2 overflow-x-auto border-b border-[#EEE5E9] bg-white px-5 py-4">
              ...
              <article className="flex-1 overflow-y-auto p-7">{(() => { const outgoing = selectedMessage.sender.id === currentUserId; const contact = outgoing ? selectedMessage.recipients[0] : selectedMessage.sender; const photo = personPhoto(contact); return <div className="flex items-start gap-4">{photo ? <img src={photo} alt={contact?.name || "Profilo"} className="size-12 shrink-0 rounded-full object-cover" /> : <div className="grid size-12 shrink-0 place-items-center rounded-full bg-[#F3DFE8] text-sm font-black text-[#8E315D]">{initials(contact?.name || "P")}</div>}<div className="min-w-0 flex-1"><h1 className="font-serif text-3xl font-semibold text-[#211A1E]">{selectedMessage.subject}</h1><div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-black/45"><strong className="text-sm text-[#30252A]">{outgoing ? `A: ${selectedMessage.recipients.map((recipient) => recipient.name).join(", ")}` : selectedMessage.sender.name}</strong><span>{outgoing ? selectedMessage.recipients.map((recipient) => recipient.email).join(", ") : `<${selectedMessage.sender.email}>`}</span><span>·</span><span>{new Intl.DateTimeFormat("it-IT", { dateStyle: "medium", timeStyle: "short" }).format(new Date(selectedMessage.createdAt))}</span></div><p className="mt-1 text-[11px] font-semibold text-black/35">Da: {selectedMessage.sender.name}</p></div></div>; })()}
                <div className="mt-8 whitespace-pre-wrap text-[15px] font-medium leading-7 text-[#493D43]">{selectedMessage.body}</div>
          ...
          ) : <div className="grid h-full place-items-center text-center"><div><Mail className="mx-auto size-10 text-black/12" /><p className="mt-4 text-sm font-black text-black/35">Seleziona un messaggio oppure crea una nuova email</p></div></div>}
        </main>
      </div>
      <style jsx>{`.mail-action{display:inline-flex;min-height:40px;flex-shrink:0;align-items:center;gap:6px;border-radius:12px;padding:0 10px;font-size:11px;font-weight:800;color:#51434a;transition:.15s}.mail-action:hover{background:#fff0f7;color:#9e3262}`}</style>
    </div>
  );
}
