...
export function LocationManager({ initialLocations, canManage }: { initialLocations: LocationItem[]; canManage: boolean }) {
  ...
  return (
    <>
      ...
      {message ? <p className="mb-4 rounded-2xl bg-paradise-nude px-4 py-3 text-sm font-semibold">{message}</p> : null}
      <section className="grid gap-5 xl:grid-cols-2">
        {locations.map((location) => (
          <Card key={location.id} className={`space-y-5 ${location.active ? "" : "opacity-65"}`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="grid size-11 place-items-center rounded-2xl bg-paradise-softPink">
                  <MapPin className="size-5" />
                </div>
                <div>
                  <h2 className="text-xl font-semibold">{location.name}</h2>
                  <p className="text-sm text-black/50">{location.address || "Indirizzo non impostato"}</p>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-black/5 bg-white/55 p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-black/45">{location.users.length} persone</span>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {location.users.map((user) => (
                  <div key={user.id} className="rounded-xl bg-white px-3 py-2 text-sm">
                    <p className="font-semibold">{user.name}</p>
                    <p className="text-xs text-black/45">{user.role}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/25 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-xl">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-black/40">Salone</p>
                <h2 className="mt-2 text-2xl font-semibold">{creating ? "Nuovo salone" : "Modifica salone"}</h2>
              </div>
              <button className="grid size-10 place-items-center rounded-xl border border-black/10" onClick={() => { setEditing(null); setCreating(false); }}><X className="size-5" /></button>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="space-y-2">
                ...
                <select className="min-h-12 w-full rounded-2xl border border-black/10 bg-white/80 px-4 text-sm" value={editing.active ? "active" : "inactive"} onChange={(event) => setEditing({ ...editing, active: event.target.value === "active" })}>
                  <option value="active">Attivo</option>
                  <option value="inactive">Eliminato / inattivo</option>
                </select>
              </label>
              <label className="space-y-2">
...
function Info({ icon: Icon, label, value }: { icon: typeof Clock3; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-black/5 bg-white/55 p-3">
      <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-black/40"><Icon className="size-4" /> {label}</p>
...
function Action({ href, icon: Icon, label, detail }: { href: string; icon: typeof Download; label: string; detail: string }) {
  return (
    <a href={href} className="rounded-2xl border border-black/5 bg-white/65 p-4 transition hover:bg-white">
      <p className="flex items-center gap-2 text-sm font-semibold"><Icon className="size-4 text-paradise-noir/65" /> {label}</p>
