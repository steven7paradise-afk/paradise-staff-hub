      ...
      <section className="rounded-[24px] border border-white/80 bg-white/80 p-5 shadow-[0_12px_40px_rgba(69,38,52,0.07)] backdrop-blur-xl lg:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <button type="button" onClick={() => router.push("/orders?status=COMPLETED")} className="mt-1 min-h-11 rounded-lg text-left text-xl font-black transition hover:text-[#9d315f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9d315f] motion-reduce:transition-none">Clienti completati per ora</button>
          </div>
          <button type="button" onClick={() => router.push("/orders?status=COMPLETED")} className="min-h-11 rounded-xl px-3 text-left transition hover:bg-[#fff0f6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9d315f] motion-reduce:transition-none sm:text-right"><strong className="text-3xl font-black">{data.clientsToday}</strong><p className="text-xs text-[#6f676b]">clienti oggi · apri ordini</p></button>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {data.hourlyClients.length === 0 ? <p className="text-sm text-[#8d8589]">Nessun controllo cliente completato oggi.</p> : data.hourlyClients.map((item) => (
            <button key={item.hour} type="button" onClick={() => router.push("/orders?status=COMPLETED")} aria-label={`Apri ordini completati delle ${item.hour}: ${item.count} clienti`} className="min-h-28 rounded-[16px] border border-[#eee4e8] bg-white/70 p-4 text-left transition duration-200 hover:-translate-y-0.5 hover:border-[#d75489] hover:bg-[#fff8fb] hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9d315f] motion-reduce:transform-none motion-reduce:transition-none">
