  ...
  return (
    <>
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        {message ? <p className="rounded-2xl bg-paradise-nude px-4 py-3 text-sm font-semibold">{message}</p> : null}
      </div>
      <Card className="overflow-hidden p-0">
        {logs.length === 0 ? <p className="p-5 text-sm text-black/50">Nessuna timbratura registrata.</p> : null}
        {logs.map((log) => (
          <div key={log.id} className="grid gap-4 border-b border-black/5 p-5 last:border-b-0 md:grid-cols-[1fr_0.65fr_0.7fr_0.7fr_1fr_auto] md:items-center">
            <div>
              <p className="font-semibold">{log.employee}</p>
              <p className="text-sm text-black/50">{log.location}</p>
            </div>
            ...
            <div>
              <p className="text-sm text-black/50">{log.device}</p>
              {log.note ? <p className="line-clamp-1 text-xs text-black/45">{log.note}</p> : null}
            </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/25 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-xl">
            <div className="mb-5 flex justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-black/40">Correzione Admin</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 sm:col-span-2">
                ...
                <Select value={draft.userId} onChange={(event) => setDraft({ ...draft, userId: event.target.value })}>
                  {workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name} - {worker.location}</option>)}
                </Select>
              </label>
