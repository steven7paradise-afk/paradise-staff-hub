...
import { Check, Search, ShieldCheck, Table2 } from "lucide-react";
import { Button, Card, Field } from "@/components/ui";
import type { AssistanceTablesAccess } from "@/lib/assistance-tables";
...
export function TablesAccessSettings({
  initialAccess,
  users,
}: {
  initialAccess: AssistanceTablesAccess;
  users: UserOption[];
}) {
  ...
  function save(next: AssistanceTablesAccess) {
    setAccess(next);
    setStatus("Salvataggio...");
    startTransition(async () => {
      try {
        const response = await fetch("/api/settings/tables-access", {
          method: "PUT",
    ...
    <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
      <Card className="rounded-[22px] p-5 hover:translate-y-0">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#B85B68]">Permessi Tabelle</p>
            <h2 className="mt-2 text-2xl font-black">Chi può vedere</h2>
            <p className="mt-2 text-sm leading-6 text-black/55 dark:text-white/55">
              Scegli ruoli o persone specifiche. Chi è abilitato vedrà la pagina Tabelle nel menu e potrà aprirla.
            </p>
          </div>
        </div>

        <div className="mt-6 rounded-2xl border border-black/5 bg-white/70 p-4 dark:border-white/10 dark:bg-white/5">
          <label className="flex cursor-pointer items-center justify-between gap-4">
            <span>
              <span className="block font-bold">Mansione assistenza</span>
              <span className="text-xs text-black/45 dark:text-white/45">Abilita automaticamente chi ha “assistenza” nella mansione.</span>
            </span>
            <input
              type="checkbox"
              checked={access.allowMansioneAssistenza}
              onChange={(event) => save({ ...access, allowMansioneAssistenza: event.target.checked })}
              className="size-5 accent-paradise-pink"
            />
          </label>
        </div>
